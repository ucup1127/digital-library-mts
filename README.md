...
## 📋 Yang Ada di README Ini

| Bagian | Isi |
|---|---|
| **Header** | Nama, deskripsi, stack |
| **Fitur** | 4 role: Public, User, Admin, Super Admin |
| **Tech Stack** | Tabel versi |
| **Prasyarat** | Node, PostgreSQL |
| **Setup Dev** | 7 langkah lengkap |
| **Env Variables** | Tabel lengkap |
| **Arsitektur** | Auth flow, multi-tenant, route groups, maintenance |
| **Struktur Folder** | Tree lengkap |
| **Deploy Production** | 10 langkah (VPS + Nginx + SSL + PM2 + cron) |
| **Testing** | Command |
| **Lisensi & Kontak** | Info |
...
1. **Ganti seluruh isi `README.md`** dengan kode di atas
2. **Save**
3. **Cek preview** — buka di VS Code, klik kanan → **Open Preview**
4. **Lapor hasilnya**

**Kalau ada yang mau diubah** (nama developer, kontak, dll) — edit aja.

## ⚠️ Yang Perlu Dikustom

| Bagian | Cek |
|---|---|
| **Developer** | Ganti "Luthfi Yusuf Ti" kalau salah |
| **Repo URL** | Pastikan `github.com/ucup1127/digital-library-mts` bener |
| **Domain** | Ganti `perpus.sekolah.sch.id` dengan domain asli |
| **Kontak** | Tambah email kalau perlu |

## 💬 Setelah README — Lanjut Task B

**Task B = Backup JSON lengkap.**

Cek dulu:
```powershell
Get-Content "app\api\admin\auto-backup\route.ts" | Select-String "getAllBackupData|findMany"
```

**Kalau masih `findMany` manual** → perlu diganti pakai `getAllBackupData()`.
